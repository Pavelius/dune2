...
static void rectfx(int x1, int y1, int x2, int y2) {
	if(x1 < clipping.x1)
		x1 = clipping.x1;
	if(x2 > clipping.x2) {
		x2 = clipping.x2;
...
int draw::textbc(const char* string, int width, bool* line_feed) {
	if(!font)
		return 0;
	int p = -1;
	int w = 0;
...
static void str832s(int x, int y, const unsigned char* s, int h) {
	auto x1 = x;
	auto y2 = y + h;
	while(true) {
		unsigned char c = *s++;
